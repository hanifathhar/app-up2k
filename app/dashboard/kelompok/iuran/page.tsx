...
import {
  HandCoins, ChevronLeft, ChevronRight, X, Printer,
...
export default function IuranPage() {
  ...
  // Filter hanya angsuran yang punya iuran > 0
  const filtered = angsuranList.filter((item) => {
    if (Number(item.iuran) <= 0) return false;
  ...
  // Summary
  const totalIuran = filtered.reduce((acc, item) => acc + Number(item.iuran), 0);
  ...
  const executePrint = () => {
    ...
    let printData = angsuranList.filter((item) => Number(item.iuran) > 0);
    ...
    if (printYear !== "all") {
      printData = printData.filter(
        (item) =>
          new Date(item.tanggal).getFullYear() === Number(printYear)
      );
    }
    if (printMonth !== "all") {
      printData = printData.filter(
        (item) =>
          new Date(item.tanggal).getMonth() === Number(printMonth)
      );
    }
    ...
    printData.forEach((item, idx) => {
      const jumlah = Number(item.iuran);
      total += jumlah;
      rowsHtml += `
        <tr>
          <td>${idx + 1}</td>
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <h2>BUKU IURAN POKSUS UP2K PKK ${headerKelompok}<br/>DESA SUGI KECAMATAN MARANCAR</h2>
          <table>
            <thead>
              <tr>
                <th>No</th>
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b pb-4 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <HandCoins className="text-orange-500" size={26} />
            Buku Iuran
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            Rekap iuran anggota dari data angsuran pinjaman.
          </p>
        </div>
        <Button
          variant="outline"
          onClick={() => setShowPrintModal(true)}
          className="bg-white border-orange-300 text-orange-700 hover:bg-orange-50 w-full sm:w-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-orange-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-orange-600 font-medium">Total Iuran Terkumpul</p>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-medium">Jumlah Pembayar</p>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-medium">Total Transaksi Iuran</p>
            <h3 className="text-xl font-bold text-gray-800">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <h3 className="font-medium text-gray-700">Daftar Pembayaran Iuran</h3>
          <div className="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto">
            {isAdmin && (
              <select
                value={filterKelompokId}
                onChange={(e) => setFilterKelompokId(e.target.value)}
                className="h-9 bg-white text-sm border border-gray-200 rounded-md px-2 w-full md:w-48 text-gray-700 focus:ring-1 focus:ring-orange-400 focus:border-orange-400"
              >
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold text-gray-700 text-right">Jumlah Iuran</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="text-center py-8 text-gray-500">Belum ada data iuran.</td></tr>
              ) : (
                paginated.map((item, idx) => (
                  <tr key={item.id} className="border-b hover:bg-orange-50/30">
                    ...
                    <td className="px-6 py-4 text-right font-semibold text-orange-700">
                      Rp {Number(item.iuran).toLocaleString("id-ID")}
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="md:hidden p-4 space-y-3 bg-gray-50/30">
          {loading ? (
            ...
            <div className="text-center py-8 text-sm text-gray-500">Belum ada data iuran.</div>
          ) : (
            paginated.map((item, idx) => (
              <div key={item.id} className="bg-white border border-orange-100 rounded-xl p-4 shadow-sm">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <p className="text-xs text-orange-600 font-medium">Iuran</p>
      ...
      {showPrintModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="bg-gradient-to-r from-orange-500 to-orange-700 p-4 text-white flex justify-between items-center">
              <h3 className="font-bold text-lg flex items-center gap-2">
                <Printer size={18} /> Cetak Laporan Iuran
              </h3>
              <button onClick={() => setShowPrintModal(false)} className="text-orange-100 hover:text-white">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="bg-gray-50 p-4 border-t flex justify-end gap-3">
              ...
              <Button onClick={executePrint} className="bg-orange-600 hover:bg-orange-700 text-white flex items-center gap-2">
